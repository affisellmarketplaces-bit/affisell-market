...
export async function recordExpansionBounceEvent(
  eventType: string,
  data: ResendWebhookEmailData,
  emailId: string
): Promise<void> {
  if (eventType !== "email.bounced" || !isExpansionBuyerResendEmail(data)) return

  const id = `expansion:bounce:${emailId}`
