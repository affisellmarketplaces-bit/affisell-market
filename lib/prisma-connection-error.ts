/** Neon Query Engine not warmed / singleton race (common right after client_created). */
export function isNeonEngineNotReadyError(error: unknown): boolean {
  return /Engine was empty|not yet connected/i.test(prismaErrorMessage(error))
}
...
export function isRetryablePrismaConnectionError(error: unknown): boolean {
  if (isNeonEngineNotReadyError(error)) return true
