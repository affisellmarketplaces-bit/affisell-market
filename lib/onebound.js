...
const ONEBOUND_ENDPOINT = "https://api-gw.onebound.cn/1688/item_get/"
...
export async function get1688(url) {
  ...
  // OneBound exige souvent key + secret (compte payant) ; optionnel sinon
  const secret = process.env.ONEBOUND_SECRET
  const params = new URLSearchParams({ key, num_iid: numIid })
  if (secret) params.set("secret", secret)

  const apiUrl = `${ONEBOUND_ENDPOINT}?${params.toString()}`
