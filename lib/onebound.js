 ...
 * Endpoint via ONEBOUND_URL (ex. …/1688global/item_get pour le cross-border, appels gratuits).
 * Docs: https://open.onebound.cn (clé/secret dans .env.local + Vercel)
 */

const DEFAULT_ENDPOINT = "https://api-gw.onebound.cn/1688global/item_get/"
...
export async function get1688(url) {
  ...
  const params = new URLSearchParams({ key, secret, num_iid: numIid, lang: "en" })
  ...
  // 1688 classique → data.item ; 1688global renvoie parfois result.item
  const item = data?.item ?? data?.result?.item
  if (!item) {
  ...
  return {
    ...
    supplier: String(item.seller_info?.shop_name ?? item.nick ?? item.seller ?? "").trim(),
