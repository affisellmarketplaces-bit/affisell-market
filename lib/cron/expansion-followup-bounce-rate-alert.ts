...
export type RunExpansionFollowupBounceRateAlertResult = {
  checked: number
  alerted: number
  countries: string[]
}
...
/** Slack/Discord when J+2 follow-up email bounce rate exceeds 5% (min 10 sent). */
export async function runExpansionFollowupBounceRateAlert(
  now = new Date()
): Promise<RunExpansionFollowupBounceRateAlertResult> {
  const weekKey = alertWeekKey(now)
  const followupStats = await loadExpansionFollowupEmailStatsByCountry(now)

  if (followupStats.size === 0) {
  ...
  for (const [countryIso2, stats] of followupStats) {
    const input = {
    ...
    const id = `expansion:followup-bounce-rate-alert:${MARKET_REGION}:${countryIso2}:${weekKey}`
    ...
    const text = `📬📉 *${countryName} (${countryIso2})* J+2 follow-up email bounce rate *${ratePct}%* (${stats.bouncesThisMonth} bounce(s) / ${stats.sentCount} sent). <${adminUrl}|Open expansion console>`
    ...
    logBusiness("expansion-rollout", {
      ...
      result: "followup_bounce_rate_alert_sent",
      ratePct,
      bouncesThisMonth: stats.bouncesThisMonth,
      sentCount: stats.sentCount,
      slack,
  ...
  return { checked: followupStats.size, alerted, countries }
}
