...
export type RunExpansionFollowupComplaintAlertResult = {
  checked: number
  alerted: number
  countries: string[]
}
...
/** Slack/Discord when a country has J+2 follow-up complaint(s) this month (min 10 sent). */
export async function runExpansionFollowupComplaintAlert(
  now = new Date()
): Promise<RunExpansionFollowupComplaintAlertResult> {
  const weekKey = alertWeekKey(now)
  const [followupSentGroups, followupComplaints] = await Promise.all([
  ...
  for (const row of followupSentGroups) {
    ...
    const complaintsThisMonth = followupComplaints.get(countryKey) ?? 0
    const input = {
      complaintsThisMonth,
    ...
    const id = `expansion:followup-complaint-alert:${MARKET_REGION}:${row.countryIso2}:${weekKey}`
    ...
    const complaintsExportUrl = `${resolveAppUrl()}${expansionComplaintsExportPath(row.countryIso2, "checkout-launch-followup")}`
    const text = `📬🚫 *${countryName} (${row.countryIso2})* J+2 follow-up complaint — *${complaintsThisMonth}* this month (${ratePct}% of sent). <${complaintsExportUrl}|Export J+2 complaints CSV> · <${adminUrl}|Expansion console>`
    ...
    logBusiness("expansion-rollout", {
      ...
      marketRegion: MARKET_REGION,
      result: "followup_complaint_alert_sent",
      complaintsThisMonth,
      ratePct,
