...
import { assessTestDatabaseUrl, dbEndpointOf, pointAtTestDatabase } from "@/lib/testing/db-test-guard"
...
describe("pointAtTestDatabase (env files)", () => {
  ...
  it("refuses staging by default, accepts it only with DB_TEST_ALLOW_STAGING=1", () => {
    ...
    expect(() => pointAtTestDatabase(repo({ ...base, ".env.test.local": `DATABASE_URL_TEST="${STAGING}"\n` }))).toThrow(/protected/)
    try {
      const ep = pointAtTestDatabase(repo({ ...base, ".env.test.local": `DATABASE_URL_TEST="${STAGING}"\nDB_TEST_ALLOW_STAGING=1\n` }))
  ...
  it("never accepts production, even with the staging flag", () => {
    ...
    expect(() => pointAtTestDatabase(dir)).toThrow(/protected/)
  })
})
