...
describe("auth public i18n (Lot H)", () => {
  ...
  it("exposes reseller signup chooser and merchant auth copy in FR/EN", () => {
    ...
    expect(fr.auth.affiliateExpress.success.titleFallback).toMatch(/succès/i)
    expect(en.auth.affiliateExpress.success.cta).toMatch(/storefront/i)
    expect(fr.auth.affiliateExpress.signingIn).toBeTruthy()
