...
export default async function LoginSelectorPage({ searchParams }: Props) {
  ...
  if (session?.user?.id && role) {
    redirect(resolvePostLoginRedirect(role, callbackUrl))
  }

  const t = await getTranslations("auth.loginSelector")
  const affiliateHref = callbackUrl
    ? `/login/affiliate?callbackUrl=${encodeURIComponent(callbackUrl)}`
    : "/login/affiliate"
