...
function findNextOrPreviousField(items, name, index) {
	const prefix = `${name}.`;
	const fields = items?.filter((item) => item.type === "field" && (item.name === name || item.name.startsWith(prefix)));
	const nextField = fields?.find((field) => getArrayFieldIndex(field.name, name) > index);
	if (nextField) return nextField;
	return fields?.reverse().find((field) => getArrayFieldIndex(field.name, name) < index);
}
