"use strict";Object.defineProperty(exports, "__esModule", {value: true});

var _DWSXHZYMcjs = require('./DWSXHZYM.cjs');






var _7FULUV5Lcjs = require('./7FULUV5L.cjs');

// ../ariakit-react-components/dist/heading/heading.js
var _react = require('react');
var _jsxruntime = require('react/jsx-runtime');
var TagName = "h1";
var useHeading = _7FULUV5Lcjs.createHook.call(void 0, function useHeading2(props) {
  const ref = _react.useRef.call(void 0, null);
  const level = _react.useContext.call(void 0, _DWSXHZYMcjs.HeadingContext) || 1;
  const Element = `h${level}`;
  const tagName = _7FULUV5Lcjs.useTagName.call(void 0, ref, Element);
  const isNativeHeading = !!tagName && /^h\d$/.test(tagName);
  props = {
    render: /* @__PURE__ */ _jsxruntime.jsx.call(void 0, Element, {}),
    role: !isNativeHeading ? "heading" : void 0,
    "aria-level": !isNativeHeading ? level : void 0,
    ...props,
    ref: _7FULUV5Lcjs.useMergeRefs.call(void 0, ref, props.ref)
  };
  return props;
});
var Heading = _7FULUV5Lcjs.forwardRef.call(void 0, function Heading2(props) {
  return _7FULUV5Lcjs.createElement.call(void 0, TagName, useHeading(props));
});




exports.useHeading = useHeading; exports.Heading = Heading;
