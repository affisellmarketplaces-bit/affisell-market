...
export function MobileDock() {
  ...
  return (
    <nav
      aria-label={t("aria")}
      className={cn(
        "affisell-mobile-buyer-dock pointer-events-none fixed inset-x-0 bottom-0 z-[90] w-full max-w-[100vw] overflow-x-clip px-3 pb-[max(0.4rem,env(safe-area-inset-bottom))] md:hidden",
    ...
    >
      ...
      >
        {dockItems.map(({ id, href, labelKey, featured, match }) => {
          ...
          return (
            <li key={id} className={cn("flex flex-1 justify-center", featured && "-mt-3")}>
              <FastLink
                href={href}
                className={cn(
