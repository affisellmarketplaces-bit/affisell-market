...
export function RadarImportBar({ selectedIds, country, supplierKind, onClear }: Props) {
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (destination === "supplier_draft" && data.redirectUrl) {
        toast.success(`${imported} produit${imported > 1 ? "s" : ""} prêt${imported > 1 ? "s" : ""} — ouverture assistant…`)
        onClear()
