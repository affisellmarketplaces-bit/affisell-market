...
import { buildRadarPricingCards, radarPricingTrust } from "@/lib/radar/pricing-copy"
...
export default async function RadarMarketingLanding() {
  const t = await getTranslations("radarLanding")
  const tShell = await getTranslations("radarShell")
  const locale = (await getLocale()) as AppLocale
  const PAID_RADAR_CARDS = buildRadarPricingCards(locale).filter(
    ...
    <div className="min-h-screen bg-[#070b14] text-zinc-100">
      ...
      <section className="mx-auto mt-12 grid max-w-5xl gap-4 px-4 sm:grid-cols-3">
        {[
          ...
          {
            t: t("card3Title"),
            d: t("card3Body", { unlock: radarGlobalUnlockLabel({ short: true }, locale) }),
          },
        ].map((c) => (
      ...
      <section className="mx-auto mt-12 max-w-5xl px-4">
        <div className="rounded-xl border border-zinc-700 bg-zinc-900 p-5 font-mono text-xs text-emerald-200 sm:text-sm">
          <p className="text-zinc-500">{t("slackExampleTitle")}</p>
          <p className="mt-2">{t("slackExampleLine")}</p>
          <p className="mt-1 text-zinc-400">
            {t("slackExampleDetail")}
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto mt-12 max-w-5xl px-4 pb-20">
        <div className="text-center">
          ...
          <h2 className="mt-2 text-2xl font-semibold tracking-tight">{t("subscriptionTitle")}</h2>
            ...
            return (
              ...
              >
                ...
                <p className="relative mt-3 text-3xl font-semibold">
                  {formatRadarPlanPrice(p.id, { includeSuffix: false }, locale)}
                  <span className="ml-1 text-sm font-normal text-zinc-500">{tShell("perMonth")}</span>
                </p>
