...
export default function RadarAppShell({ children }: { children: React.ReactNode }) {
  ...
  if (status === "loading" || !session?.user) {
    return <div className="mx-auto max-w-3xl px-4 py-16 text-sm text-zinc-500">Chargement…</div>
  }
  ...
  const nav = [
    ...
    { href: "/radar/connect", label: "Scanner un marketplace" },
  ]
      ...
      <header className="border-b border-zinc-200 bg-white">
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-4 px-4 py-4">
          <div>
            ...
            <h1 className="text-lg font-semibold text-zinc-900">Affisell Radar</h1>
          </div>
