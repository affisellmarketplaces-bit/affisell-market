...
import {
  ...
  buildWizardV2AeImportBookmarklet,
...
export function WizardV2AeRelayClient({
  relayKey,
  sessionId,
  captureToken,
  aeUrl,
  ...
  const bookmarkletHref = useMemo(
    () => buildWizardV2AeImportBookmarklet(appOrigin, relayKey, sessionId, captureToken),
    ...
    (payload: ImportPayload) => {
      ...
      if (window.opener && !window.opener.closed) {
        window.opener.postMessage(
          {
            type: "AFFISELL_WIZARD_AE_IMPORT_DONE",
            relayKey,
            sessionId,
      ...
      setStatus("done")
      setDetail("Produit capturé — retour au wizard…")
  ...
  const startPolling = useCallback(() => {
    ...
    pollRef.current = window.setInterval(() => {
      ...
      void fetch(
        `/api/supplier/wizard-v2/ae-capture/${encodeURIComponent(relayKey)}/poll?sessionId=${encodeURIComponent(sessionId)}&consume=1`,
        { credentials: "include" }
      )
        .then((r) => r.json())
        .then((data: { ready?: boolean; import?: Record<string, unknown> }) => {
          if (!data.ready || !data.import) return
          notifyOpenerAndClose({ import: data.import })
        })
        .catch(() => {})
    }, POLL_MS)
  }, [notifyOpenerAndClose, relayKey, sessionId, stopPolling])
  ...
  const openAliExpress = useCallback(() => {
    const aeWin = window.open("about:blank", "affisellWizardAe")
    if (!aeWin) {
      setStatus("error")
