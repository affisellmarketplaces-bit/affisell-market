...
import {
  ...
  type WizardV2AeImportData,
} from "@/components/supplier/wizard-v2/wizard-v2-ae-browser-bridge"
...
import { resolveWizardV2Mode, type WizardV2Mode } from "@/lib/product-wizard-v2/feature-flag"
import {
...
function WizardProTab({ ownerUserId }: { ownerUserId: string }) {
  return <SupplierAddProductForm ownerUserId={ownerUserId} embeddedInWizardV2 />
}

type ExpressTabProps = {
  expressUrl: string
  setExpressUrl: (v: string) => void
  publishing: boolean
  runExpressImport: () => void
  showAeBrowserBridge: boolean
  aeBrowserAutoStart: boolean
  setPublishing: (v: boolean) => void
  applyExpressImportData: (data: WizardV2AeImportData) => void
  images: string[]
        ...
        <div className="space-y-4 border-t border-zinc-200 pt-6 dark:border-zinc-800">
          {images.length === 0 ? (
            ...
            <div className="flex gap-2 overflow-x-auto pb-1">
              ...
              {images.length > 12 ? (
                <span className="flex h-16 items-center text-xs text-zinc-500">+{images.length - 12}</span>
              ) : null}
            </div>
          )}
          <div>
  ...
  const openFullWizardPrefilled = useCallback(() => {
    ...
    writeSupplierAddProductDraftCache(ownerUserId, {
      ...
      descriptionIllustrationImages:
        patch?.illustrationImages?.length ? patch.illustrationImages : images.slice(1, 24),
      ...
      variantFormMode: hasAdvancedSkus
        ...
        : patch?.variants.mode === "simple"
          ? "simple"
          : "none",
      variantSizesText: patch?.variants.mode === "simple" ? patch.variants.sizes.join(", ") : "",
      variantColorsText:
    ...
    <BentoShell>
      <div className="mx-auto max-w-6xl px-4 py-8 md:px-8">
        <WizardHubTabs
          ...
          onModeChange={setModeInUrl}
        />

        <div key={mode} className="animate-in fade-in duration-200">
          {mode === "pro" ? (
            <WizardProTab ownerUserId={ownerUserId} />
          ) : (
            <WizardExpressTab
              expressUrl={expressUrl}
              setExpressUrl={setExpressUrl}
              publishing={publishing}
              runExpressImport={() => void runExpressImport()}
              showAeBrowserBridge={showAeBrowserBridge}
              aeBrowserAutoStart={aeBrowserAutoStart}
              setPublishing={setPublishing}
              applyExpressImportData={applyExpressImportData}
              images={images}
              setImages={setImages}
              ...
              setCategoryBreadcrumb={setCategoryBreadcrumb}
              browse={browse}
              skuVariants={skuVariants}
              catalogPriceEur={catalogPriceEur}
              commissionPct={commissionPct}
              handleApplySmartMargin={handleApplySmartMargin}
