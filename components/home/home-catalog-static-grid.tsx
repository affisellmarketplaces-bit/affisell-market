...
export async function HomeCatalogStaticGrid({ shell, limit = 20 }: Props) {
  ...
  const items = shell.products
    ...
    // Skip ghost shells: no displayable title → would leave empty grid slots.
  ...
  return (
    <div id="explorer" className="affisell-home-explorer min-w-0">
      ...
      <p className="mb-1.5 hidden text-[11px] text-zinc-500 dark:text-zinc-400 md:block">
            ...
            <Fragment key={item.id}>
              <li className="flex h-full min-w-0">
                <CatalogCard
                  href={item.href}
                  prefetch
                  className="affisell-inp-tap affisell-home-static-card affisell-product-card-surface group flex h-full w-full touch-manipulation flex-col rounded-[1.35rem] border border-[color:var(--affisell-premium-border)] p-1 shadow-[var(--affisell-premium-shadow-soft)] transition-transform duration-200 active:scale-[0.99] sm:rounded-3xl sm:p-1.5"
                >
