        ...
        {BUYER_SMART_SERVICES.map(({ href, label, hint, Icon, accent }) => (
          <li key={href} className="min-w-0">
            <Link
              href={href}
              className={cn(
                "group relative flex h-full min-h-[5.5rem] flex-col justify-between overflow-hidden rounded-2xl border border-white/25 bg-white/10 p-3.5 shadow-lg shadow-violet-950/20 backdrop-blur-md transition duration-300",
                "hover:-translate-y-0.5 hover:border-white/40 hover:bg-white/15 hover:shadow-xl hover:shadow-violet-950/30 active:scale-[0.99] sm:min-h-[6rem] sm:p-4"
              )}
            >
              <span
                className={cn(
