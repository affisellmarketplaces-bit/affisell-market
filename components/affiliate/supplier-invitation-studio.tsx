...
const STATUS_LABEL: Record<string, string> = {
  OPEN: "Lien actif",
  REGISTERED: "Fournisseur inscrit",
  CATALOG_LIVE: "Produit en ligne",
}

export function SupplierInvitationStudio({
  affiliateDisplayName,
}: {
  affiliateDisplayName: string
}) {
  const [invitations, setInvitations] = useState<AffiliateInvitationListItem[]>([])
  ...
  const share = useMemo(() => {
    ...
    return buildSupplierInviteSharePayload({
      url: activeUrl,
  ...
  async function saveInvitation() {
    ...
    try {
      ...
      const res = activeInvitationId
        ? await fetch(`/api/affiliate/supplier-invitations/${encodeURIComponent(activeInvitationId)}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          })
        : await fetch("/api/affiliate/supplier-invitations", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          })
  ...
  function openChannel(channel: SupplierInviteShareChannel) {
    if (!share) return
    const map: Record<SupplierInviteShareChannel, string | undefined> = {
      copy: undefined,
  ...
  const shareButtons: { id: SupplierInviteShareChannel; label: string; icon: typeof Copy }[] = [
  ...
  return (
    <div className="space-y-8">
      <BentoPageHeading
        ...
        description="Un lien unique : inscription, premier produit, et notifications automatiques pour vous et le réseau affilié."
      />

      <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <BentoCard className="space-y-5 border-violet-100/80 dark:border-violet-900/40">
          ...
          <button
            ...
            className={cn(
              buttonVariants(),
              "h-11 w-full gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white hover:from-violet-500 hover:to-fuchsia-500"
            )}
          >
        ...
        <BentoCard className="flex flex-col gap-4 bg-gradient-to-br from-zinc-50 to-violet-50/50 dark:from-zinc-900 dark:to-violet-950/30">
          <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-100">Partager sur tous vos canaux</p>
          {activeUrl ? (
            <div className="rounded-2xl border border-dashed border-violet-300/60 bg-white/80 p-3 dark:border-violet-800 dark:bg-zinc-950/80">
              <p className="break-all font-mono text-[11px] leading-relaxed text-violet-900 dark:text-violet-200">
                {activeUrl}
              </p>
              {activeToken ? (
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {shareButtons.map((b) => (
              <button
                ...
                className={cn(
                  "flex items-center justify-center gap-1.5 rounded-xl border border-zinc-200 bg-white py-2.5 text-xs font-medium transition hover:border-violet-300 hover:bg-violet-50 disabled:opacity-40 dark:border-zinc-700 dark:bg-zinc-900 dark:hover:border-violet-700 dark:hover:bg-violet-950/50"
                )}
              >
          ...
          <ul className="mt-auto space-y-2 border-t border-zinc-200/80 pt-4 text-xs text-zinc-600 dark:border-zinc-800 dark:text-zinc-400">
            <li className="flex gap-2">
              <Users className="mt-0.5 h-3.5 w-3.5 shrink-0 text-violet-600" aria-hidden />
              Vous êtes notifié dès l&apos;inscription du fournisseur.
            </li>
            <li className="flex gap-2">
              <Store className="mt-0.5 h-3.5 w-3.5 shrink-0 text-violet-600" aria-hidden />
              À la publication du 1er SKU, tout le réseau affilié est alerté (commission, produit).
            </li>
          </ul>
        </BentoCard>
      </div>
