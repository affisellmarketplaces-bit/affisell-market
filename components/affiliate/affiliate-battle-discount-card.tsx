...
function ListingTile({
  ...
  const borderColor = selected
    ? side === "A"
      ? "border-amber-500 ring-2 ring-amber-400/40 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-400"
      : "border-violet-500 ring-2 ring-violet-400/40 bg-violet-50 dark:bg-violet-950/30 dark:border-violet-400"
    : "border-zinc-200 bg-zinc-50/80 hover:border-zinc-300 dark:border-zinc-800 dark:bg-zinc-900/40 dark:hover:border-zinc-700"
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        "relative flex w-full items-center gap-3 rounded-xl border p-2.5 text-left transition disabled:opacity-50",
        borderColor
      )}
    ...
    <div className="space-y-4 rounded-2xl border border-violet-300/60 bg-gradient-to-br from-violet-50/80 via-white to-amber-50/60 p-5 dark:border-violet-800/50 dark:from-violet-950/40 dark:via-zinc-950 dark:to-amber-950/20">
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
        <div className="rounded-xl border border-amber-200 bg-white p-3 text-center dark:border-amber-800/50 dark:bg-zinc-900">
          {pickA.imageUrl ? (
            <div className="relative mx-auto mb-2 size-16 overflow-hidden rounded-lg">
              <Image src={pickA.imageUrl} alt="" fill className="object-cover" sizes="64px" unoptimized />
            </div>
          ) : null}
          <p className="line-clamp-2 text-xs font-semibold text-zinc-900 dark:text-zinc-100">{pickA.title}</p>
          <p className="mt-1 text-[11px] text-zinc-500 line-through">{formatStoreCurrencyFromCents(pickA.priceCents)}</p>
          <p className="text-sm font-bold text-amber-600 dark:text-amber-400">{formatStoreCurrencyFromCents(priceA)}</p>
        </div>
        ...
        <div className="rounded-xl border border-violet-200 bg-white p-3 text-center dark:border-violet-800/50 dark:bg-zinc-900">
          {pickB.imageUrl ? (
            <div className="relative mx-auto mb-2 size-16 overflow-hidden rounded-lg">
              <Image src={pickB.imageUrl} alt="" fill className="object-cover" sizes="64px" unoptimized />
            </div>
          ) : null}
          <p className="line-clamp-2 text-xs font-semibold text-zinc-900 dark:text-zinc-100">{pickB.title}</p>
          <p className="mt-1 text-[11px] text-zinc-500 line-through">{formatStoreCurrencyFromCents(pickB.priceCents)}</p>
          <p className="text-sm font-bold text-violet-600 dark:text-violet-400">{formatStoreCurrencyFromCents(priceB)}</p>
        </div>
      </div>
...
/**
 * Full Battle command panel — dual select, discount, duration, preview, end/delete.
 */
export function AffiliateBattleDiscountCard() {
  ...
  const syncFromBattle = useCallback(
    (b: BattleState, catalog: ListingPick[]) => {
      setBattleState(b)
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (battleRes.ok) {
          const data = (await battleRes.json().catch(() => null)) as { battle?: BattleState | null } | null
          const b = data?.battle
  ...
  function toggleProduct(listingId: string) {
    ...
    if (!selectedA) {
      ...
      if (listings.find((l) => l.listingId === listingId)?.productId ===
          listings.find((l) => l.listingId === selectedA)?.productId) {
        setError("Les deux produits doivent être différents")
        return
      }
  ...
  const pickA = useMemo(() => listings.find((l) => l.listingId === selectedA) ?? null, [listings, selectedA])
  const pickB = useMemo(() => listings.find((l) => l.listingId === selectedB) ?? null, [listings, selectedB])
  const canPreview = Boolean(pickA && pickB && pickA.productId !== pickB.productId)

  function goPreview() {
    if (!canPreview) {
      setError("Sélectionne 2 produits différents pour le duel")
      return
    }
    setError(null)
    setPhase("preview")
  ...
  async function launchBattle() {
    ...
    try {
      ...
      setBattleState({
        ...
        productA: pickA ? { id: pickA.productId, name: pickA.title, image: pickA.imageUrl, priceCents: pickA.priceCents } : undefined,
        productB: pickB ? { id: pickB.productId, name: pickB.title, image: pickB.imageUrl, priceCents: pickB.priceCents } : undefined,
      })
      setPhase("live")
      setStatusMsg("Battle lancé")
      console.log("[pulse-battle/hub]", { result: "launched", battleId: nextId, selectedA, selectedB, durationMinutes })
    } catch (e) {
  ...
  return (
    <section id="battle" className="mx-auto max-w-3xl scroll-mt-24 space-y-3 px-4 pt-4 sm:px-6">
      ...
      {loading ? (
            ...
            {phase === "live" ? (
              <>
                <span className="mr-auto inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-700 dark:text-emerald-300">
                  ...
                  En direct
                  {battleState?.votesA != null && battleState?.votesB != null ? (
                    <span className="ml-1 font-normal text-zinc-500">
                      — {(battleState.votesA ?? 0) + (battleState.votesB ?? 0)} votes
                    </span>
                  ) : null}
                </span>
                <Button
        ...
        /* ── Select phase ── */
        <div className="space-y-4 rounded-2xl border border-violet-200/80 bg-white p-4 dark:border-violet-900/50 dark:bg-zinc-950 sm:p-5">
          <div>
            <h2 className="flex items-center gap-2 text-sm font-bold text-zinc-900 dark:text-white">
              ...
              Configure ton Battle
            </h2>
            <p className="mt-1 text-xs text-zinc-600 dark:text-zinc-400">
              Coche <strong>2 produits</strong> de ta vitrine pour le duel. Le gagnant
              (votes acheteurs) reçoit le flash discount.
            </p>
          </div>
            ...
            <>
              <div className="flex flex-wrap gap-3">
                <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-amber-600 dark:text-amber-400">
                  <span className="flex size-5 items-center justify-center rounded-full bg-amber-500 text-[9px] text-white">A</span>
                  {pickA?.title ? (
                    <span className="max-w-[8rem] truncate">{pickA.title}</span>
                  ) : (
                    "Produit A"
                  )}
                </div>
                ...
                <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-violet-600 dark:text-violet-400">
                  <span className="flex size-5 items-center justify-center rounded-full bg-violet-500 text-[9px] text-white">B</span>
                  {pickB?.title ? (
                    <span className="max-w-[8rem] truncate">{pickB.title}</span>
                  ) : (
                    "Produit B"
                  )}
                </div>
              ...
              <ul className="grid max-h-52 gap-2 overflow-y-auto sm:grid-cols-2" data-testid="battle-product-picker">
                {listings.map((item) => {
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-[10px] text-zinc-500">
              <span>5%</span>
              <span className="font-bold text-violet-600 dark:text-violet-400">−{flashDiscount}%</span>
              <span>50%</span>
            </div>
          </div>
